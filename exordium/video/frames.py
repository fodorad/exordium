...
def frames2video(input_dir: str, output_path: str | Path, fps: int = 25):
...
def video2frames(input_path: str | Path,
                 output_dir: str | Path,
                 fps: int = 25,
                 smallest_dim: int = 360,
    ...
    if crop is None:
        CMD = f'ffmpeg -loglevel panic -i {str(input_path)} -r {fps} -vf scale={h}:{w} {str(output_dir)}/frame_%05d.png -nostdin -vf -an -hide_banner'
    else:
        cy, cx, cw, ch = crop
        CMD = f'ffmpeg -loglevel panic -i {str(input_path)} -r {fps} -vf "scale={h}:{w}:flags=neighbor,crop={ch}:{cw}:{cx}:{cy}" {str(output_dir)}/frame_%05d.png -nostdin -vf -an -hide_banner'
