...
from exordium.utils.ckpt import download_file, download_weight
...
class SwinTWrapper(VisualModelWrapper):
...
def window_partition(x, window_size):  # pragma: no cover
...
def window_reverse(windows, window_size, H, W):  # pragma: no cover
...
class WindowAttention(nn.Module):  # pragma: no cover
    ...
    def forward(self, x, mask=None):
        ...
        relative_position_bias = self.relative_position_bias_table[
            self.relative_position_index.view(-1)  # ty: ignore[call-non-callable]
        ].view(
    ...
    def extra_repr(self) -> str:
        ...
        Returns:
            String with dimension, window size and num heads.
        """
...
class SwinTransformerBlock(nn.Module):  # pragma: no cover
...
class PatchMerging(nn.Module):  # pragma: no cover
    ...
    def extra_repr(self) -> str:
        ...
        Returns:
            String with input resolution and dimension.
        """
...
class BasicLayer(nn.Module):  # pragma: no cover
    ...
    def extra_repr(self) -> str:
        ...
        Returns:
            String with dimension, resolution and depth.
        """
    ...
    def flops(self):
        ...
        flops = 0
        for blk in self.blocks:
            flops += blk.flops()  # ty: ignore[call-non-callable]
        if self.downsample is not None:
...
class SwinTransformer(nn.Module):  # pragma: no cover
    r"""Swin Transformer hierarchical vision transformer.
...
def swin_transformer_tiny(pretrained=True, **kwargs):
    ...
    Args:
        pretrained: If True, load ImageNet pre-trained weights. Defaults to True.
        **kwargs: Additional arguments for SwinTransformer.
    ...
    if pretrained:
        local_path = download_weight(model_name["swin_transformer_tiny"], WEIGHT_DIR / "swin")
        model.load_state_dict(
            torch.load(local_path, map_location=torch.device("cpu"), weights_only=True)["model"]
        )
    return model
...
def swin_transformer_small(pretrained=True, **kwargs):
    ...
    if pretrained:
        remote_path = f"https://github.com/SwinTransformer/storage/releases/download/v1.0.0/{model_name['swin_transformer_small']}"
        local_path = WEIGHT_DIR / "swin" / Path(remote_path).name
...
def swin_transformer_base(pretrained=True, **kwargs):
    ...
    if pretrained:
        remote_path = f"https://github.com/SwinTransformer/storage/releases/download/v1.0.0/{model_name['swin_transformer_base']}"
        local_path = WEIGHT_DIR / "swin" / Path(remote_path).name
        download_file(remote_path, local_path)
        model.load_state_dict(
            torch.load(local_path, map_location=torch.device("cpu"), weights_only=True)["model"]
...
_SWIN_FACTORY = {
    "tiny": swin_transformer_tiny,
    "small": swin_transformer_small,
    "base": swin_transformer_base,
}
"""Mapping of Swin Transformer size names to their constructor functions."""
