...
def swin_transformer_tiny(pretrained=True, **kwargs):
    ...
    if pretrained:
        remote_path = f"https://github.com/fodorad/exordium/releases/download/v1.0.0/{model_name['swin_transformer_tiny']}"
        local_path = WEIGHT_DIR / "swin" / Path(remote_path).name
        download_file(remote_path, local_path)
        model.load_state_dict(torch.load(local_path, map_location=torch.device("cpu"))["model"])
    return model
