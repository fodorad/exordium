...
from exordium.video.core.bb import iou_xywh
...
from exordium.video.core.io import interpolate_1d
...
class Tracker(ABC):
    ...
    Provides multi-object tracking across video frames using detection associations.
    """
    ...
    def selected_tracks(self) -> dict[int, Track]:
        """Get the selected tracks or all tracks if none selected.
    ...
    def label(
        ...
        """Assign detections to tracks across frames.

        Args:
            detections: VideoDetections containing per-frame detections.
            min_score: Minimum confidence score to consider. Defaults to 0.7.
            max_lost: Maximum frame gap to maintain track continuity. Defaults to 30.
            iou_threshold: IoU threshold for matching detections to tracks. Defaults to 0.2.

        Returns:
            Self with labeled tracks.
        """
        # iterate over frame-wise detections
        for frame_detections in tqdm(
            detections, total=len(detections), desc="Label tracks", disable=not self.verbose
        ):
            # iterate over detections within a frame
            for detection in frame_detections:
                # ignore low confidence detections
                if detection.score < min_score:  # pragma: no cover
                    continue
                ...
                for _, track in self.tracks.items():
                    ...
                    # calculate iou of the detection and the frame distance
                ...
                if candidate_tracks:
                    ...
                    best_candidate_id, _ = sorted(
                        candidate_tracks, key=lambda x: x[1], reverse=True
                    )[0]
    ...
    def merge(self) -> Self:
        """Merge similar tracks based on merge rule.

        Returns:
            Self with merged tracks.
        """
    ...
    def _interpolate_detections(self) -> Self:
        for _, track in self.tracks.items():
            ...
            for ind in indices:
                ...
                new_bb = np.rint(
                    interpolate_1d(
                        frame_id_start, frame_id_end, detection_start.bb_xywh, detection_end.bb_xywh
                    )
                ).astype(int)

                # interpolate lmks coords
                new_landmarks_x = interpolate_1d(
                    frame_id_start,
                    frame_id_end,
                    detection_start.landmarks[:, 0],
                    detection_end.landmarks[:, 0],
                )
                new_landmarks_y = interpolate_1d(
                    frame_id_start,
                    frame_id_end,
                    detection_start.landmarks[:, 1],
                    detection_end.landmarks[:, 1],
                )
                new_landmarks = np.rint(
                    np.stack([new_landmarks_x, new_landmarks_y], axis=2)
                ).astype(int)  # (B, N) -> (B, N, 2)
                ...
                for ind, frame_id in enumerate(new_frame_ids):
                    ...
                    source: str | np.ndarray = (
                        str(Path(detection_start.source).parent / self.id_to_path(frame_id))
                        if isinstance(detection_start, DetectionFromImage)
                        else detection_start.source
                    )

                    # source frame or video does not exist, then skip it entirely.
                    if isinstance(source, str) and not Path(source).exists():  # pragma: no cover
    ...
    def select_long_tracks(self, min_length: int = 250) -> Self:
        """Select tracks with length greater than minimum.

        Args:
            min_length: Minimum track length. Defaults to 250.

        Returns:
            Self with selected tracks.
        """
    ...
    def select_topk_long_tracks(self, top_k: int = 1) -> Self:
        """Select top-k longest tracks.

        Args:
            top_k: Number of tracks to select. Defaults to 1.
    ...
    def select_topk_biggest_bb_tracks(self, top_k: int = 1) -> Self:
        """Select top-k tracks with largest bounding boxes.
    ...
    def get_center_track(self) -> Track | None:
        """Get track closest to image center.

        Returns:
            Track closest to center, or None if no tracks.
        """
        sorted_tracks = sorted(
            [track for _, track in self.selected_tracks.items()],
    ...
    def __init__(self, verbose: bool = False, max_lost: int = -1, iou_threshold: float = 0.2):
        ...
        Args:
            ...
            max_lost (int, optional): maximum acceptable frame distance between tracks.
                -1 means that it any number of frames between two tracks are accepted.
                Defaults to -1.
            iou_threshold (float, optional): intersection over union threshold
                of two detections from the tracks. Defaults to 0.2.
