...
def xyxy2xywh(bb_xyxy: np.ndarray) -> np.ndarray:
    ...
    Args:
        bb_xyxy: Bounding box in xyxy format (xmin, ymin, xmax, ymax).

    Returns:
        Bounding box in xywh format (xmin, ymin, width, height).
...
def xywh2xyxy(bb_xywh: np.ndarray) -> np.ndarray:
    ...
    Args:
        bb_xywh: Bounding box in xywh format (xmin, ymin, width, height).
...
def midwh2xywh(bb_midwh: np.ndarray) -> np.ndarray:
    ...
    Args:
        bb_midwh: Bounding box in midwh format (xmid, ymid, width, height).
...
def xywh2midwh(bb_xywh: np.ndarray) -> np.ndarray:
    ...
    Returns:
        Bounding box in midwh format (xmid, ymid, width, height).
...
def xyxy2full(bb_xyxy: np.ndarray) -> np.ndarray:
    """Convert bounding box to full corner coordinates.

    Args:
        bb_xyxy: Bounding box in xyxy format.

    Returns:
        Array of corner coordinates as 4x2.
    """
...
def visualize_bb(
    ...
    """Draw a bounding box and detection probability on an image.

    Args:
        image: Input image of shape ``(H, W, C)`` in BGR format.
        bb_xyxy: Bounding box ``[xmin, ymin, xmax, ymax]`` of shape ``(4,)``.
        probability: Detection confidence score to display.
        output_path: Path to save the output image. ``None`` skips saving.

    Returns:
        Image with bounding box and probability label drawn on it.

    Raises:
        Exception: If ``bb_xyxy`` does not have shape ``(4,)``.

    """
    if bb_xyxy.shape != (4,):
        raise Exception(f"Expected bounding box with shape (4,) got instead {bb_xyxy.shape}.")
