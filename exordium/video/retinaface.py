...
from exordium.video.bb import xywh2xyxy, xyxy2xywh


class RetinafaceDetections():

    def __init__(self, format: str = 'dict'):
        assert format in {'dict', 'xyxys,lmks'}
        self.format = format
        self.detections = []
    ...
    def __getitem__(self, idx):
        ...
        if self.format == 'xyxys,lmks':
            return (list(xywh2xyxy(self.detections[idx]['bb'])) + [self.detections[idx]['score']], 
                    self.detections[idx]['landmarks'])

        return self.detections[idx]

    def get_highest_score(self, frame_ind: int):
        det = sorted([det for det in self.detections if det['frame'] == frame_ind], key=lambda d: d['score'])[0]

        if self.format == 'xyxys,lmks':
            return (list(xywh2xyxy(det['bb'])) + [det['score']], det['landmarks'])
...
def detect_faces(frame_paths: list | str,
    ...
    detections = RetinafaceDetections(format=format)
