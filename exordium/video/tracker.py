...
from exordium.utils.shared import timer_with_return, load_or_create
from exordium.video.bb import iou_xywh, xywh2xyxy
from exordium.video.retinaface import detect_faces, RetinafaceDetections
...
class Track():
    ...
    def __init__(self, id: int, detection: dict, verbose: bool = False):
        ...
        if verbose: print(f'{detection["frame"]}: new track ({self.id}) is started')
...
def face_visualization(frames: list, tracks: OrderedDict, output_dir: str, sample_every_n: int = 1, extra_percent: float = 0.2):
    ...
    if output_dir.exists(): return # skip already done samples
    ...
    for frame_ind, frame_path in tqdm(enumerate(frames), total=len(frames)):
        ...
        for _, track in tracks.items():
            ...
            if frame_ind not in track.location:
                ...
                cv2.imwrite(str(output_dir / 'frame_{:05d}.png'.format(frame_ind)), frame)
            else:
                detection = track.location[frame_ind]
                bb_xyxy = xywh2xyxy(detection['bb'])
                ...
                cv2.imwrite(str(output_dir / 'frame_{:05d}.png'.format(frame_ind)), face)
