...
def face_alignment(img, left_eye, right_eye, nose):
    ...
    upside_down = False
    if nose[0] < left_eye[0] or nose[0] < right_eye[0]:
        upside_down = True
    ...
    a = np.linalg.norm(np.array(right_eye)  - np.array(point_3rd))
    b = np.linalg.norm(np.array(left_eye) - np.array(point_3rd))
    c = np.linalg.norm(np.array(left_eye) - np.array(right_eye))

    if b != 0 and c != 0: # division by zero
