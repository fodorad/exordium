...
class Tag:
    ...
    def __init__(
        ...
        self.timestamp_path = (Path(timestamp_path) if timestamp_path
                               is not None else self.tag_path.parent /
                               f"{self.tag_path.stem}.txt")
        self.video_path = (Path(video_path) if video_path is not None else
                           self.tag_path.parent / f"{self.tag_path.stem}.avi")
        self.frames_dir = (Path(frames_dir) if frames_dir is not None else
                           self.tag_path.parent / "frames")
        self.extension = extension

        assert self.tag_path.exists(), f"File was not found: {self.tag_path}"
        assert (self.timestamp_path.exists()
                ), f"File was not found: {self.timestamp_path}"
        assert self.video_path.exists(
        ), f"File was not found: {self.video_path}"
        assert (self.frames_dir.is_dir()
                ), f"Directory of the frame is invalid: {self.frames_dir}"
        ...
        self.frame_timestamp = {
            ...
            for frame_id, timestamp in frame_timestamp
        }

        self.frame_paths = [
            self.frames_dir / name
        ...
        assert (
            start is not None
        ), f"Invalid #start token index ({start}). Check the annotation file: {self.tag_path}"
        assert (
            end is not None
        ), f"Invalid #end token index ({end}). Check the annotation file: {self.tag_path}"
        assert (
            end > start
        ), f"Invalid intervals. #end ({end}) should be bigger than #start ({start}). Check the annotation file: {self.tag_path}"
        ...
        for line in lines[start + 1:end]:
            ...
            assert len(parts) == len(
                keys
            ), f"Invalid line. Length of the record is {len(parts)}, while the length of the keys is {len(keys)}"

            record = {
                k: keys_type[i](v)
                for i, (k, v) in enumerate(dict(zip(keys, parts)).items())
            }
            ...
            self.annotation.append({
                ...
                "bb_xywh":
                np.array([
                    record["F_X"], record["F_Y"], record["F_W"], record["F_H"]
                ]),
                "left_eye_xy":
                np.array([
                    record["LE_LX"],
        ...
        if self.annotation[0]["frame_id"] == 0 and int(
                self.frame_paths[0].stem) == 1:
        ...
        self.annotation = list(
            filter(
                lambda record:
                (self.frames_dir / f'{record["frame_id"]:06d}.png').exists(),
                self.annotation,
            ))
        ids, counts = np.unique([elem["blink_id"] for elem in self.annotation],
                                return_counts=True)
    ...
    def __getitem__(self, frame_id: int) -> dict:
        ...
        """
        annotation = next(
            (elem for elem in self.annotation if elem["frame_id"] == frame_id),
            None)

        if annotation is None:
            raise ValueError(
                f"None value is returned. There is no frame with the requested frame_id ({frame_id})"
            )
    ...
    def first_frame(self, blink_id: int) -> dict:
        ...
        """
        annotation = next(
            (elem for elem in self.annotation if elem["blink_id"] == blink_id),
            None)

        if annotation is None:
            raise ValueError(
                f"None value is returned. There is no annotation with the requested blink_id ({blink_id})"
            )
    ...
    def print(self) -> None:
        ...
        unique, counts = np.unique(
            [elem["blink_id"] for elem in self.annotation], return_counts=True)

        for u, c in zip(unique, counts):
            print("value:", u, "count:", c)

        print("Binary annotation stats:",
              np.unique(self.binary(), return_counts=True))
    ...
    def get_positive_sample(
        ...
        frame_paths = [
            str(Path(self.frames_dir) / f"{frame_id:06d}.png")
        ...
        if output_dir is not None:
            frames2video(
                ...
                output_path=str(
                    Path(output_dir) /
                    f"{self.video_id}_{frame_ids[0]}_{frame_ids[-1]}.mp4"),
                fps=fps,
            )
            ...
            for frame_path, gt in zip(frame_paths, list(labels)):
                output_dir_frame = (
                    Path(output_dir) /
                    f"{self.video_id}_{frame_ids[0]}_{frame_ids[-1]}")
                output_dir_frame.mkdir(parents=True, exist_ok=True)
                cv2.imwrite(
                    str(output_dir_frame /
                        f"{Path(frame_path).stem}_{gt}.png"),
    ...
    def generate_positive_samples(
        ...
        positive_samples = []
        for blink_id in tqdm(blink_ids,
                             desc="Generating positive (blink) samples"):
            positive_samples.append(
                self.get_positive_sample(blink_id,
                                         win_size=win_size,
    ...
    def generate_negative_samples(
        ...
        with tqdm(total=num_samples,
                  desc="Generating negative (non-blink) samples") as pbar:
    ...
    def crop_face(self, frame: np.ndarray,
                  bb_xywh: np.ndarray | list) -> np.ndarray:
    ...
    def crop_eyes(
        ...
        right_cx = right_eye_xy[0] + abs(right_eye_xy[0] -
                                         right_eye_xy[2]) // 2
    ...
    def save_annotated_face_crops(self, output_dir: str | Path) -> None:
        ...
        for annotation in tqdm(self.annotation,
                               total=len(self.annotation),
                               desc="crop faces"):
            if not all([elem == 0 for elem in list(annotation["bb_xywh"])]):
                img = cv2.imread(
                    str(self.frames_dir / f'{annotation["frame_id"]:06d}.png'))
                face = self.crop_face(img, annotation["bb_xywh"])
                cv2.imwrite(
                    str(output_dir / f'{annotation["frame_id"]:06d}.png'),
                    face)
    ...
    def save_annotated_eye_crops(self, output_dir: str | Path) -> None:
        ...
        for annotation in tqdm(self.annotation,
            ...
            if not all([elem == 0 for elem in list(annotation["bb_xywh"])]):
                img = cv2.imread(
                    str(self.frames_dir / f'{annotation["frame_id"]:06d}.png'))
                left_eye, right_eye = self.crop_eyes(
                    img, annotation["left_eye_xy"], annotation["right_eye_xy"])
                cv2.imwrite(
                    str(output_dir / "left" /
                        f'{annotation["frame_id"]:06d}.png'),
                    left_eye,
    ...
    def map_annotation_to_frames_decord(self, output_dir: str | Path) -> None:
        ...
        timestamps = np.array(vr.get_frame_timestamp(range(
            len(vr))))[:, 1]  # [[start, end],...]
        for frame_id, gt_timestamp in tqdm(
                self.frame_timestamp.items(),
                total=len(self.frame_timestamp),
                desc="Extract frames",
        ):
